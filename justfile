...
test:
    eval "$(luarocks path --tree {{rock_tree}} --lua-version 5.1 --bin)" && nvim --headless -u NONE -c 'set rtp+=.' -c 'lua package.path = "{{nvim_package_path}}" .. package.path; arg = { "spec/config_spec.lua", "spec/keymaps_spec.lua", "spec/lazy_spec.lua", "spec/tty_spec.lua", "spec/navigation_spec.lua" }; require("busted.runner")({ standalone = false })' -c qall
